...
class QueryBuilder
{
    ...
    {
        return $this->join('NATURAL JOIN `'._DB_PREFIX_.$table.'`'.($alias ? ' `'.$alias.'`' : ''));
    }
    ...
    {
        return $this->join('RIGHT JOIN `'._DB_PREFIX_.$table.'`'.($alias ? ' `'.$alias.'`' : '').($on ? ' ON '.$on : ''));
    }
    ...
    public function build()
    {
        ...
        if (!$this->query['from']) {
            echo('Table name not set in QueryBuilder object. Cannot build a valid SQL query.');
        }
