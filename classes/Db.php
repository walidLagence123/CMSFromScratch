...
abstract class Db
{
    ...
    public static function getClass()
    {
        ...
        if (empty($class)) {
            echo('Cannot select any valid SQL engine.');
        }
    ...
    public function insert($table, $data, $null_values = false, $use_cache = true, $type = Db::INSERT, $add_prefix = true)
    {
        ...
        if ($type == Db::INSERT) {
            ...
            echo('Bad keyword, must be Db::INSERT or Db::INSERT_IGNORE or Db::REPLACE');
        }
            ...
            foreach ($row_data as $key => $value) {
                if (!$first_loop) {
                    ...
                    if (!in_array("`$key`", $keys)) {
                        echo('Keys form $data subarray don\'t match');
                    }

                    if ($duplicate_key_stringified != '') {
                        echo('On duplicate key cannot be used on insert with more than 1 VALUE group');
                    }
                } else {
                    $keys[] = '`'.$key.'`';
                }
                ...
                if ($type == Db::ON_DUPLICATE_KEY) {
                    $duplicate_key_stringified .= '`'.$key.'` = '.$string_value.',';
                }
            }
            $first_loop = false;
    ...
    public function executeS($sql, $array = true, $use_cache = true)
    {
        ...
        if (!preg_match('#^\s*\(?\s*(select|show|explain|describe|desc)\s#i', $sql)) {
            if (defined('_PS_MODE_DEV_') && _PS_MODE_DEV_) {
                echo('Db->executeS() must be used only with select, show, explain or describe queries');
            }
